...
Every prop you used above — `bg`, `p`, `gap`, `mt`, `shadow`, `variant` — comes from Gamut's shared [system props](/foundations/system-props/) or a component's own variant list, which is why they work the same way across components. The [Components](/components/) reference documents each one, and each component page's **Prop Reference** section shows every value a prop like `shadow` or `variant` can take.
