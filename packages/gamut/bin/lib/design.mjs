...
/**
 * @param {string} sourceRoot @skillsoft/gamut-agent-tools package root
...
export async function installDesignMd(sourceRoot, cwd, theme, options = {}) {
  ...
  const pkg = await readFile(join(sourceRoot, 'package.json'), 'utf8')
    .then((raw) => JSON.parse(raw))
    .catch(() => null);
  const pkgName = pkg?.name ?? '@skillsoft/gamut-agent-tools';
  const version = pkg?.version ?? 'unknown';

  const header =
    `<!-- Generated by ${pkgName}@${version}.\n` +
