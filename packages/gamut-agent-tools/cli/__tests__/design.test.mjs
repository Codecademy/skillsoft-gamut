...
async function makeSourceRoot(dir, pkgVersion = '71.0.0') {
  ...
  // Minimal package.json at the root of the agent-tools package itself
  if (pkgVersion !== null) {
    await writeFile(
      join(sourceRoot, 'package.json'),
      JSON.stringify({
        name: '@skillsoft/gamut-agent-tools',
        version: pkgVersion,
      }),
      'utf8'
    );
...
describe('installDesignMd', () => {
  ...
  it('stamps the gamut version in an HTML comment at the top of DESIGN.md', async () => {
    ...
    assert.match(
      content,
      /^<!-- Generated by @skillsoft\/gamut-agent-tools@99\.0\.0\./
    );
  });
  ...
  it('falls back to "unknown" version when package.json is missing', async () => {
    ...
    assert.match(
      content,
      /^<!-- Generated by @skillsoft\/gamut-agent-tools@unknown\./
    );
  });
