...
<Callout
  text={
    <>
      Import <code>Video</code> from <code>@skillsoft/gamut/Video</code> — it is
      not included in <code>@skillsoft/gamut</code>'s default export.
    </>
  }
/>
...
The `Video` component is a video player that supports multiple video sources and formats with enhanced accessibility features.
