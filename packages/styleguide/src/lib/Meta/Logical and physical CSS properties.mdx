...
In custom components, read the same flag Emotion-styled Gamut components use via `useLogicalProperties()` from `@skillsoft/gamut-styles`:
...
`useElementDir` resolves the effective text direction (`'rtl'` or `'ltr'`) for a DOM subtree. Import it from `@skillsoft/gamut-styles`:
