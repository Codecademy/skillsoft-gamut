...
`@codecademy/gamut` brings in `gamut-styles`, `gamut-icons`, `gamut-illustrations`, `gamut-patterns`, and `variance` automatically as regular dependencies, so no separate install is needed for those.
